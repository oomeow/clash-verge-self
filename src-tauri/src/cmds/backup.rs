...
use crate::{
    cmds::{self, into_command_result},
    config::Config,
    core::backup::{self, BackupFile, BackupType, WebDav},
};

#[tauri::command]
pub async fn create_backup(backup_type: BackupType, only_backup_profiles: bool) -> Result<(String, PathBuf), String> {
...
#[tauri::command]
pub async fn list_backup(backup_type: BackupType) -> Result<Vec<BackupFile>, String> {
    into_command_result(backup::list_backup_files(backup_type).await)
}

#[tauri::command]
pub async fn delete_backup(backup_type: BackupType, file_name: String) -> Result<(), String> {
    into_command_result(backup::delete_backup_file(backup_type, file_name).await)
}

// web dav
#[tauri::command]
pub async fn update_webdav_info(url: String, username: String, password: String) -> Result<(), String> {
